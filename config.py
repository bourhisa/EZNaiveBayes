...
TWO_AHEAD = False  # (n, n+2)
THREE_CONSECUTIVE = False # (n, n+1, n+1+2)
TWO_RANDOM = False # All the combinations of 2 words within the phrase

# Dataset parameters

DATASET_PATH = 'datasets/beta/'  # Where the datasets are stored
ORIGINAL_CSV = DATASET_PATH + 'dataset.csv'  # Original CSV  file
FILE_JSON = DATASET_PATH + 'subs/dataset.json'  # Optional JSON file. Cleaned version of the CSV
FILE_TRAINING = DATASET_PATH + 'subs/dataset.json' # Training set, all classifiers built upon this one
FILE_TESTING = DATASET_PATH + 'subs/dataset.json'  # Testing set, where the test items will be picked
...
INDEX_LABEL = 0 # Index of the label in the CSV  input
INDEX_TEXT = 5  # Index of the text
...
LIST_SAMPLES = [50000, 100000, 500000]  # Sizes of the different classifiers
TO_BE_TRAINED = LIST_SAMPLES  # Samples to be trained
TO_BE_TESTED = LIST_SAMPLES  # Samples to be tested

LABELS = []  # List of all labels found in the training dataset, fills itself automatically

SIZE_OF_SAMPLE_TEST = 10000  # Number of items used to run the test
